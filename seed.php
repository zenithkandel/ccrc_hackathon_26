...
$insertContrib->execute([
    'type' => 'route', 'entry_id' => $pendingRouteId, 'agent' => $agentIds[0],
    'status' => 'pending', 'proposed_at' => date('Y-m-d H:i:s', strtotime('-2 days')),
]);
...
$insertContrib->execute([
    'type' => 'vehicle', 'entry_id' => $pendingVehicleId, 'agent' => $agentIds[1],
    'status' => 'pending', 'proposed_at' => date('Y-m-d H:i:s', strtotime('-1 day')),
]);
...
foreach ($alerts as $a) {
    ...
    foreach ($a['routes'] as $rn) {
        if (isset($routeIds[$rn])) $affected[] = $routeIds[$rn];
    }
    $insertAlert->execute([
        'name'    => $a['name'],
        'desc'    => $a['desc'],
...
$suggestions = [
    [
        'type' => 'appreciation', 'rating' => 5, 'status' => 'reviewed',
        ...
        'route' => 'Sajha Line 1: Lagankhel - Gongabu', 'ip' => '192.168.1.101',
    ],
    [
        'type' => 'suggestion', 'rating' => 4, 'status' => 'pending',
        'message' => 'Please add night bus routes. Sajha stops at 9 PM but many people work late in Thamel and need to get to Lagankhel.',
        'route' => null, 'ip' => '192.168.1.102',
    ],
    [
        'type' => 'complaint', 'rating' => 2, 'status' => 'pending',
        ...
        'route' => 'Micro: Kalanki - Ratnapark', 'ip' => '192.168.1.103',
    ],
    [
        'type' => 'correction', 'rating' => 3, 'status' => 'pending',
        'message' => 'Sajha electric bus on Line 3 now starts from Shahid Gate, not Ratnapark. Schedule changed since December.',
        'route' => 'Sajha Line 3: Ratnapark - Airport', 'ip' => '192.168.1.104',
    ],
    [
        'type' => 'appreciation', 'rating' => 5, 'status' => 'reviewed',
...
foreach ($trips as $i => $t) {
    ...
    foreach ($t['routes'] as $rn) {
        if (isset($routeIds[$rn])) $usedRoutes[] = $routeIds[$rn];
    }
    if (!isset($locationIds[$t['from']]) || !isset($locationIds[$t['to']])) continue;
